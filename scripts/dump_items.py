...
INPUT_DIR = Path("api-data/data/api/v2/item")
...
for path in sorted(INPUT_DIR.glob("*/*.json")):
    ...
    item_id = data.get("id", path.parent.name)
...
with open("dump/items/items.data.json", "w", encoding="utf-8") as f:
    json.dump(items, f, ensure_ascii=False, indent=4)

with open("dump/items/item.names.map.json", "w", encoding="utf-8") as f:
